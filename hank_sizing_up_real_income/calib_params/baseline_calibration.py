...
calibrated_parameters = {
    ...
    'sigma': 1.0,  # Elasticity of intertemporal substitution (EIS)
    ...
    'markup_ss': 1.015,  # Steady-state markup of prices over marginal cost
    'theta_w': 0.66,  # Wage stickiness parameter (Calvo parameter for wages)
    ...
    'sd_e': 0.57,  # Standard deviation of productivity shocks
    'n_e': 7,  # Number of discrete income states in the Markov chain

    # Macro Aggregates
    'Y': 1.0,  # Aggregate output (normalized to 1)
    'A': 1,  # Aggregate assets
    'C': 1,  # Aggregate consumption
    'C_RA': 1.0,  # Representative agent's consumption (used in RA model)
    ...
    'X': 1.0,  # Export quantity (normalized steady state value)

    # Monetary Policy
    'r_star': 0.04,  # Foreign interest rate (steady state value)
    'r_steady_state': 0.04,  # Domestic steady-state real interest rate
    'pi': 0.0,  # Inflation rate (steady state assumed to be 0)
    'i_policy_shock': 0.0,  # Monetary policy shock (default to no shock)
    ...
    'lambd': 0.75  # Share of hand-to-mouth agents in the economy
}
